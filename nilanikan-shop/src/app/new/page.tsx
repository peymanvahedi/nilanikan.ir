...
export default async function Page() {
  ...
  const items = listify(home?.newArrivals).map(toProduct);

  return (
    <main className="container mx-auto px-3 sm:px-4 md:px-6 py-6" dir="rtl">
      <div className="mb-4 flex items-center justify-between">
        <h1 className="text-2xl md:text-3xl font-extrabold text-slate-900">جدیدترین‌ها</h1>
        <Link
          href="/"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
        {items.map((it: any, i: number) => (
          <ProductCard
            key={it.id ?? it.slug ?? i}
            item={{ ...it, ribbon: it.ribbon ?? "جدید", ribbonTone: it.ribbonTone ?? "pink" }}
            hrefBase="/product"
          />
        ))}
      </div>
