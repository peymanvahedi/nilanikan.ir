...
export default function Header() {
  ...
  const CartIcon = useMemo(() => (
    <svg viewBox="0 0 24 24" className="w-6 h-6">
      <path d="M7 4h-2l-1 2v2h2l3.6 7.59-1.35 2.44A1 1 0 0 0 9 20h10v-2H10.42l.93-1.68h7.45a1 1 0 0 0 .92-.62l3-7A1 1 0 0 0 22 7H6.21l-.94-2H2V3h4l1.6 3H22" fill="currentColor" />
    </svg>
  ), []);
    ...
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur border-b border-zinc-200" dir="rtl">
      ...
      <div className="hidden md:flex mx-auto max-w-6xl px-4 h-20 items-center justify-between gap-4">
        <div className="shrink-0 flex items-center gap-2">
          <Link
            href="/"
            className="text-3xl font-black text-pink-600 leading-none"
            data-tour data-tour-title="خانه فروشگاه" data-tour-content="با کلیک روی لوگو همیشه به صفحه اصلی برمی‌گردید." data-tour-placement="bottom" data-tour-order="1"
          >
            نیلانیکان
          </Link>

          <button
            onClick={() => start()} // یا start({skipIntro:true})
            className="help-btn hidden lg:inline-flex items-center gap-1 h-9 px-3 rounded-xl border border-zinc-300 text-zinc-700 hover:bg-zinc-50"
            aria-label="نمایش آموزش"
            data-tour data-tour-title="راهنمای سریع" data-tour-content="هر زمان این دکمه را بزنید، آموزش همین صفحه اجرا می‌شود." data-tour-placement="bottom" data-tour-order="1"
          >
            ❓ راهنما
          </button>
        </div>
        ...
        <form
          onSubmit={onSubmit}
          className="grow max-w-2xl w-full"
          data-tour data-tour-title="جست‌وجوی محصولات" data-tour-content="نام محصول، برند یا دسته‌بندی را تایپ کنید و اینتر بزنید." data-tour-placement="bottom" data-tour-order="2"
        >
          <div className="relative">
            <input
              value={q} onChange={(e) => setQ(e.target.value)} placeholder="جست‌وجو..."
        ...
        <div className="shrink-0 flex items-center gap-4">
          ...
          <Link
            href="/cart" className="relative inline-flex items-center text-zinc-800 hover:text-pink-600"
            data-tour data-tour-title="سبد خرید" data-tour-content="اقلام انتخاب‌شده‌تان اینجاست؛ برای رفتن به پرداخت کلیک کنید." data-tour-placement="bottom" data-tour-order="3"
          >
            {CartIcon}
            {cartCount > 0 && (
      ...
      <div className="bg-zinc-900 text-white">
        <nav className="mx-auto max-w-6xl px-4 hidden md:block">
          <div
            className="flex items-center gap-6 overflow-x-auto no-scrollbar py-3 text-sm"
            data-tour data-tour-title="مرور دسته‌بندی‌ها" data-tour-content="برای دسترسی سریع به بخش‌های محبوب از این نوار استفاده کنید." data-tour-placement="top" data-tour-order="4"
          >
            {categories.map((c) => (
